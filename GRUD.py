...
class GRUD(nn.Module):
    def __init__(self, input_size, hidden_size, X_mean, num_class = 2, output_last = False):
        ...
        # Alter output to hidden size
        # Altered this from Delta size to transfor to hidden zie
    ...
    def step(self, x, x_last_obsv, x_mean, h, mask, delta):
        ...
        # Determination what measurement to present to the step
        # If there is a measurement present mask (1) * x will present the measurement if present
        # if not present then the opposite will happen (1-mask) activates 2nd half of equation  
        ...
        # Need to alter the tensors to float 32 
        h = delta_h * h
        # Not sure whether this should be axis = 0 or 1
        combined = torch.cat((x, h, mask), 1).to(torch.float32)
        z = F.sigmoid(self.zl(combined))
        r = F.sigmoid(self.rl(combined))
        combined_r = torch.cat((x, r * h, mask), 1).to(torch.float32) # Resetgate
        # print('Help: ', combined_r.dtype)
        h_tilde = F.tanh(self.hl(combined_r)) # Candidate hidden state
        
        h = (1 - z) * h + z * h_tilde # updategate
